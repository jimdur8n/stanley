...
$queryCol = "SELECT DISTINCT relation FROM techsmith ORDER BY relation ASC;";

//Selecting videos with search terms in the title or description, and with the collection picked.
$query = "SELECT * FROM techsmith WHERE (description LIKE '%".$search."%' OR title LIKE '%".$search."%' OR collection LIKE '%".$search."%' OR subject LIKE '%".$search."%' OR relation LIKE '%".$search."%' OR reference LIKE '%".$search."%') AND relation LIKE '%".$topic_button."%' ORDER BY date ASC limit ".$offset.", ".$page_result.";";
...
echo '<select name="topic_button" title="select a topic">';
	...
	echo '<option value="'.$collection["relation"].'">'.$collection["relation"].'</option>';
	}
echo '</select>';
...
foreach ($temp as $item){
		...
		echo '<a target="_blank" href="viewer.php?id='.$item['ID'].'">';
